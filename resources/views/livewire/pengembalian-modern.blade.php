...
<div style="padding: 28px;">
    ...
    <div style="background: linear-gradient(135deg, #3b82f6 0%, #2563eb 100%); padding: 24px 28px; border-radius: 16px; margin-bottom: 28px; box-shadow: 0 4px 12px rgba(37, 99, 235, 0.2);">
        ...
        <p style="color: rgba(255, 255, 255, 0.9); margin: 8px 0 0 40px; font-size: 14px;">
            {{ $activeCount }} peminjaman aktif
            @if($overdueCount > 0)
                <span style="color: #fef3c7; font-weight: 600;"> • {{ $overdueCount }} terlambat</span>
            @endif
        </p>
    </div>
            ...
            <div style="display: flex; align-items: start;">
                ...
                <div style="flex: 1;">
                    ...
                    <div style="color: #374151; font-size: 13px; line-height: 1.8;">
                        <strong>Keterlambatan:</strong> Rp {{ number_format($tarif_denda_per_hari, 0, ',', '.') }}/hari/buku •
                        <strong>Rusak:</strong> Rp {{ number_format($tarif_denda_rusak, 0, ',', '.') }} •
                        <strong>Hilang:</strong> Rp {{ number_format($tarif_denda_hilang, 0, ',', '.') }}
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="pengembalian-card-modern">
        ...
        @endphp
        <div class="pengembalian-list-item {{ $terlambat ? 'terlambat' : '' }}">
            <div class="row align-items-center">
                {{-- Column 1: Info Icon & Transaction --}}
                <div class="col-md-5">
                    <div style="display: flex; align-items: center; gap: 14px;">
                        <div style="background: linear-gradient(135deg, {{ $terlambat ? '#ef4444 0%, #dc2626' : '#3b82f6 0%, #2563eb' }} 100%); width: 44px; height: 44px; border-radius: 12px; display: flex; align-items: center; justify-content: center; flex-shrink: 0;">
                            <i data-feather="{{ $terlambat ? 'alert-circle' : 'file-text' }}" style="width: 22px; height: 22px; color: white;"></i>
                        </div>
                ...
                {{-- Column 2: Dates & Books --}}
                <div class="col-md-4">
                    <div style="font-size: 12px; color: {{ $terlambat ? '#991b1b' : '#6b7280' }}; line-height: 1.8;">
                        ...
                        <div>
                            ...
                            <strong>{{ $data->jumlah_peminjaman }}</strong> buku
                        </div>
                    </div>
                </div>
                ...
                <div class="col-md-3">
                    <div style="display: flex; align-items: center; justify-content: space-between; gap: 10px;">
                        ...
                        @if($isPustakawan)
                        <button class="pengembalian-btn-action" data-toggle="modal" data-target="#returnModal{{ $data->id_peminjaman }}" wire:click="openReturnForm({{ $data->id_peminjaman }})">
    ...
    <div class="pengembalian-card-modern" style="margin-top: 20px;">
        <div style="padding: 16px 24px; display: flex; justify-content: space-between; align-items: center;">
            <div style="color: #6b7280; font-size: 13px;">
                Menampilkan <strong style="color: #374151;">{{ $peminjaman->firstItem() ?? 0 }}</strong> - 
                <strong style="color: #374151;">{{ $peminjaman->lastItem() ?? 0 }}</strong> dari 
                <strong style="color: #374151;">{{ $peminjaman->total() }}</strong> peminjaman
            </div>
            <nav>
                {{ $peminjaman->links() }}
            </nav>
...
<div wire:ignore.self class="modal fade show" id="returnModal{{ $selectedPeminjaman->id_peminjaman }}" tabindex="-1" role="dialog" style="display: block; background: rgba(0,0,0,0.5);" aria-modal="true">
            ...
            <div class="modal-body" style="padding: 28px; background: #f9fafb;">
                ...
                <div style="background: #f3f4f6; padding: 20px; border-radius: 12px; margin-bottom: 20px;">
                    ...
                    <div class="row">
                        <div class="col-md-6" style="margin-bottom: 12px;">
                            ...
                            <div style="font-size: 14px; color: #374151; font-weight: 600;">{{ $selectedPeminjaman->kode_transaksi }}</div>
                        </div>
                        <div class="col-md-6" style="margin-bottom: 12px;">
                            ...
                            <div style="font-size: 14px; color: #374151; font-weight: 600;">
                                ...
                                <span style="background: linear-gradient(135deg, #3b82f6 0%, #2563eb 100%); color: white; font-size: 10px; padding: 2px 8px; border-radius: 4px; margin-left: 6px;">{{ strtoupper($selectedPeminjaman->anggota->jenis_anggota) }}</span>
                            </div>
                        </div>
                        <div class="col-md-4" style="margin-bottom: 12px;">
                            <div style="font-size: 12px; color: #6b7280; margin-bottom: 4px;">Tanggal Pinjam</div>
                            <div style="font-size: 13px; color: #374151;">{{ \Carbon\Carbon::parse($selectedPeminjaman->tgl_pinjam)->format('d/m/Y') }}</div>
                        </div>
                        <div class="col-md-4" style="margin-bottom: 12px;">
                            <div style="font-size: 12px; color: #6b7280; margin-bottom: 4px;">Jatuh Tempo</div>
                            <div style="font-size: 13px; color: #374151;">{{ \Carbon\Carbon::parse($selectedPeminjaman->tgl_jatuh_tempo)->format('d/m/Y') }}</div>
                        </div>
                        <div class="col-md-4" style="margin-bottom: 12px;">
                ...
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 14px;">
                    <h6 style="color: #374151; font-weight: 600; margin: 0; display: flex; align-items: center; font-size: 15px;">
                        ...
                        Daftar Buku yang Dikembalikan (<span id="selectedCount">{{ count($selectedEksemplar) }}</span>/{{ $totalBelumKembali }})
                    </h6>
                    <div>
                        <button type="button" wire:click="$set('selectedEksemplar', {{ $belumDikembalikan->pluck('id_detail')->toJson() }})" style="background: #3b82f6; color: white; border: none; padding: 6px 12px; border-radius: 6px; font-size: 12px; margin-right: 6px; cursor: pointer; transition: all 0.2s;" onmouseover="this.style.background='#2563eb'" onmouseout="this.style.background='#3b82f6'">
                ...
                <div style="margin-bottom: 20px;">
                    ...
                    @foreach($selectedPeminjaman->detailPeminjaman as $detail)
                        @if(!$detail->tgl_kembali)
                            ...
                            @endphp
                            <div style="background: white; border: 2px solid {{ $isChecked ? '#3b82f6' : '#e5e7eb' }}; border-radius: 10px; padding: 16px; margin-bottom: 12px; transition: all 0.3s;">
                                <div class="row align-items-center">
                                    <div class="col-md-1" style="text-align: center;">
                                        <input type="checkbox" wire:model.live="selectedEksemplar" value="{{ $detail->id_detail }}" style="width: 20px; height: 20px; cursor: pointer; accent-color: #3b82f6;">
                                    </div>
                                    <div class="col-md-1" style="text-align: center;">
                                        <div style="background: linear-gradient(135deg, #3b82f6 0%, #2563eb 100%); width: 32px; height: 32px; border-radius: 8px; display: flex; align-items: center; justify-content: center; color: white; font-weight: 700; font-size: 13px; margin: 0 auto;">{{ $displayIndex }}</div>
                                    </div>
                                    <div class="col-md-4">
                                        <div style="font-weight: 600; color: #3b82f6; font-size: 13px; margin-bottom: 4px;">{{ $detail->eksemplar->kode_eksemplar }}</div>
                                        <div style="font-weight: 600; color: #374151; font-size: 14px; margin-bottom: 4px;">{{ $detail->eksemplar->buku->judul }}</div>
                                        <div style="font-size: 12px; color: #6b7280;">{{ $detail->eksemplar->buku->no_panggil }}</div>
                                    </div>
                                    <div class="col-md-3">
                                        <label style="color: #6b7280; font-size: 11px; margin-bottom: 6px; display: block; font-weight: 600;">Kondisi Buku</label>
                                        <select wire:model.live="detailItems.{{ $detail->id_detail }}.kondisi_kembali" style="width: 100%; padding: 8px 12px; border: 2px solid #e5e7eb; border-radius: 8px; font-size: 13px; transition: all 0.2s; background: white;" onfocus="this.style.borderColor='#3b82f6';" onblur="this.style.borderColor='#e5e7eb';">
                ...
                {{-- Ringkasan Denda (Modern Clean Style) --}}
                        ...
                        <div style="display: flex; justify-content: space-between; align-items: center;">
                            <div>
                                ...
                                <div style="font-size: 11px; color: #6b7280;">
                                    @if($hari > 0)
                                        {{ $hari }} hari × {{ count($selectedEksemplar) }} buku dipilih × Rp {{ number_format($tarif_denda_per_hari, 0, ',', '.') }}
                                    @else
                                        Tidak terlambat
                                    @endif
                                </div>
