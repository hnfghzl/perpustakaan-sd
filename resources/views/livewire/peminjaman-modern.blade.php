                    ...
                    <div class="col-md-5">
                        ...
                        <div style="background: #f3f4f6; padding: 20px; border-radius: 12px; margin-bottom: 16px;">
                            ...
                            <div style="font-size: 13px; color: #6b7280; margin-bottom: 12px; padding-left: 4px;">
                                ...
                                Ditemukan <strong style="color: #3b82f6;">{{ $anggotaList->count() }}</strong> anggota dari pencarian "{{ $searchAnggota }}"
                            </div>
                            @endif
                            ...
                            <div style="margin-bottom: 16px;">
                                ...
                                <select wire:model.live="id_anggota" style="width: 100%; padding: 12px 16px; border: 2px solid #d1d5db; border-radius: 8px; font-size: 15px; transition: all 0.2s; background: white; max-height: 200px;" onfocus="this.style.borderColor='#3b82f6'; this.style.boxShadow='0 0 0 3px rgba(59, 130, 246, 0.1)';" onblur="this.style.borderColor='#d1d5db'; this.style.boxShadow='none';">
                                    ...
                                    @if($anggotaList->count() > 0)
                                        ...
                                        <option value="{{ $a->id_anggota }}">{{ $a->nama_anggota }} ({{ $a->jenis_anggota == 'guru' ? 'Guru' : 'Siswa' }}){{ $a->institusi ? ' - ' . $a->institusi : '' }}</option>
                                        @endforeach
                                    @else
                    ...
                    <div class="col-md-7">
                        <h6 style="color: #374151; font-weight: 600; margin-bottom: 14px; display: flex; align-items: center; justify-content: space-between; font-size: 16px;">
                            ...
                            <span style="font-size: 14px; color: #6b7280; font-weight: 400;">
                                <span style="color: #3b82f6; font-weight: 600;">{{ count($selectedEksemplar) }}</span>/3 dipilih
                            </span>
                        </h6>
                        ...
                        <div style="border: 2px solid #e5e7eb; padding: 12px; max-height: 450px; overflow-y: auto; border-radius: 12px; background: white;">
                            @if($eksemplarList->count() > 0)
                                ...
                                @endphp
                                <div class="peminjaman-book-item {{ $isChecked ? 'selected' : '' }} {{ $isDisabled && !$isChecked ? 'disabled' : '' }}">
                                    <div style="display: flex; align-items: start;">
                                        <div style="flex-shrink: 0; margin-right: 12px; margin-top: 2px;">
                                            <input type="checkbox" 
                                                ...
                                                style="width: 18px; height: 18px; cursor: {{ $isDisabled && !$isChecked ? 'not-allowed' : 'pointer' }}; accent-color: #3b82f6;">
                                        </div>
                                        <label for="e{{ $e->id_eksemplar }}" style="flex: 1; cursor: {{ $isDisabled && !$isChecked ? 'not-allowed' : 'pointer' }};">
                                            <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 6px;">
                                                <span style="font-weight: 600; color: {{ $isChecked ? '#1e40af' : '#3b82f6' }}; font-size: 15px;">{{ $e->kode_eksemplar }}</span>
                                                @if($bukuSudahDipilih)
                                            ...
                                            <div style="color: {{ $isChecked ? '#1e40af' : '#374151' }}; font-size: 15px; font-weight: 600; margin-bottom: 6px;">{{ $e->buku->judul }}</div>
                                            <div style="display: flex; align-items: center; gap: 12px; font-size: 13px; color: {{ $isChecked ? '#2563eb' : '#6b7280' }};">
                                                <span style="display: flex; align-items: center;">
